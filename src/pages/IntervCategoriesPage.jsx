...
const PALETTE = [
  ...
  { color: '#2E86C1', bg: '#E3F0FA' },
  { color: '#D4860B', bg: '#FBF1E0' },
  { color: '#8E44AD', bg: '#F0E6F6' },
  { color: '#1D8348', bg: '#E6F2EC' },
  { color: '#E91E8C', bg: '#FCE4F5' },
  { color: '#5D6D7E', bg: '#EAF0F6' },
  { color: '#4A5568', bg: '#F0F2F5' },
  { color: '#0A1E3D', bg: '#E8ECF0' },
  { color: '#2ECC71', bg: '#E8FAF0' },
]
...
function CategoryModal({ category, onClose, onSave, loading }) {
  ...
  const [color, setColor] = useState(category?.color || PALETTE[0].color)
  const [bg, setBg]       = useState(category?.bg || PALETTE[0].bg)
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 28, maxWidth: 440, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 24 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {PALETTE.map(p => (
                <button key={p.color} onClick={() => { setColor(p.color); setBg(p.bg) }} style={{ width: 36, height: 36, borderRadius: 8, background: p.bg, border: `2px solid ${color === p.color ? p.color : '#E8ECF0'}`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: color === p.color ? `0 0 0 2px ${p.color}40` : 'none' }}>
          ...
          <div>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color, background: bg, borderRadius: 8, padding: '6px 14px', display: 'inline-block' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={() => onSave({ name, color, bg })} disabled={!name || loading} style={{ flex: 2, height: 42, borderRadius: 8, border: 'none', background: !name ? '#CBD5E0' : '#0A1E3D', fontSize: 13, fontWeight: 700, color: '#fff', cursor: !name ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1100, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 28, maxWidth: 360, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        ...
        <p style={{ fontSize: 13, color: '#8694A7', marginBottom: 24, lineHeight: 1.6 }}>Supprimer "{name}" ? Les interventions existantes conserveront leur catégorie en texte libre.</p>
        <div style={{ display: 'flex', gap: 10 }}>
...
export default function IntervCategoriesPage() {
  ...
  const load = () => api.get('/admin/intervention-categories').then(setCategories).finally(() => setLoading(false))
  useEffect(() => { load() }, [])
  ...
  const handleSave = async (data) => {
    ...
    try {
      if (modal === 'add') {
        const c = await api.post('/admin/intervention-categories', data)
        setCategories(prev => [...prev, c])
        showToast('Catégorie ajoutée.')
      } else {
        const c = await api.put(`/admin/intervention-categories/${modal.id}`, data)
  ...
  const handleDelete = async () => {
    setActionLoading(true)
    try {
      await api.delete(`/admin/intervention-categories/${toDelete.id}`)
  ...
  return (
    <div style={{ padding: 32 }}>
      ...
      <div style={{ background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 10, padding: '10px 14px', marginBottom: 16, fontSize: 12, color: '#2E86C1' }}>
        ℹ️ Ces catégories apparaissent lors de la création d'une intervention dans l'application.
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E8ECF0', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 160px 80px', padding: '10px 16px', background: '#F7F8FA', borderBottom: '1px solid #E8ECF0' }}>
            {['Nom', 'Aperçu', 'Actions'].map((h, i) => (
              <span key={i} style={{ fontSize: 11, fontWeight: 700, color: '#8694A7', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</span>
            ))}
          </div>
          {categories.map((cat, i) => (
            <div key={cat.id} style={{ display: 'grid', gridTemplateColumns: '1fr 160px 80px', padding: '12px 16px', borderBottom: i < categories.length - 1 ? '1px solid #F0F2F5' : 'none', background: i % 2 === 0 ? '#fff' : '#FAFAFA', alignItems: 'center' }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: '#0A1E3D' }}>{cat.name}</span>
              <div>
                <span style={{ fontSize: 11, fontWeight: 700, color: cat.color || '#4A5568', background: cat.bg || '#F0F2F5', borderRadius: 6, padding: '3px 10px' }}>
                  {cat.name}
                </span>
              </div>
